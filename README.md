...
- **Função Principal:** Cria e executa instâncias de sensores, chamando o método remoto (`sendData`) do Servidor SPS a cada 30 segundos.

| Diretório     | Descrição da Responsabilidade                                                                                                                                |
| :------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`client`**  | Contém a classe principal (`SensorClient`) que realiza a conexão com o RMI Registry, localiza o _stub_ do Servidor SPS e chama o método remoto (`sendData`). |
...
**Função Principal:** Cria e executa instâncias de sensores, chamando o método remoto (`sendData`) do Servidor SPS a cada 60 segundos.
...
2.  **Servidor SPS:** Execute o `ServerApp.java` (inicia RMI Registry 3000, RMI Services e HTTP 4567).
3.  **Clientes Sensores:** Execute o `App.java` (simula sensores enviando dados).

O _frontend_ pode então consultar o Servidor SPS (`:4567`) para os dados de medição e o Servidor de Alertas (`:4568`) para o histórico de notificações.
